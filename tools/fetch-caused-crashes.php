...
const PLUGIN_NAME = ""; // required
const MIN_PLUGIN_VERSION = ""; // required
...
do {
	...
	for($i = 0; $i < $matchCount; ++$i) {
		...
		preg_match_all("/<title>([#\w\s()\\\:\$\-._\/,=&;~!|]*)<\/title>/", $crashHtml, $matches);
...
foreach($crashesDuplicatesCount as $id => $duplicateCount) {
	out($i++ . ". [$duplicateCount] $crashesMessagesWithLinks[$id]");
}
